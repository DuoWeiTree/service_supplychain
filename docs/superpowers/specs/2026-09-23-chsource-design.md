...
| E-19 | ★★ **两张采购表的采集日集合不一样**（09-23 实测）：行项表 09-19 / 20 / **22** / 23，单据表 09-19 / 20 / 23 —— **09-22 那天行项是当天的、单据停在 09-20（两天前）**。在途 = 行项给数量 × 单据给 `status`，而陈旧守卫只量了行项表。单据表冻住 ⇒ 已完成/已作废的单继续被读成「待到货」，而 E-12 说 status=9 的行项 `quantity_receive` 全为 0，算术过滤救不了 —— 最多虚报 104,479 件（真实开口量的 3.7 倍）。今天 `purchase_staleness_days = 3` 连那次 2 天的漂移都不会红 | 守卫要量两张表里更老的那个 |
