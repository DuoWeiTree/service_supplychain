...
| E-13 | 真实开口在途（`status=2 待到货`）：166 行 / **28,561 件** / **39 个货号** / **9 张采购单**。按 `expect_arrive_time` 分月：2026-06 10,199 · 07 6,594 · 08 6,098 · 09 5,325 · **10 仅 345**，**10 月之后为 0** | 绝大多数已逾期 |
| E-13a | ★ **本表初稿把 E-13 的归属写成「全部来自 2 张采购单（PO260514011、PO260721011）」，那是错的**（终审 M-3，2026-09-23 复测确认）。真实是 9 张：PO260514011 16,793 · PO260706011 7,380 · PO260615003 1,690 · PO260721011 1,428 · 其余 5 张合计 1,270。那两张只占 18,221/28,561（63.8%）。总量与分月都对，只有归属这一句错。**09-22 与 09-23 两个快照给的是同一组 166/39/9/28,561** —— 所以不是数据漂移，是初稿就写错了。★ 记这一条是因为「全部来自 2 张单」这种句子最容易被抄进迁移文件头当依据（记忆 `ad-hoc-numbers-become-load-bearing`） | 归属要按单据重数 |
...
| E-14a | 补 E-14（09-23 复测）：那 1 行 NULL **不在开口在途集合里**（它的单据不是 `status=2`），所以今天 `missing_eta_rows` 实测为 **0**。守卫照留 —— 它防的是「哪天它落进来了要被数出来」，不是「它现在就在」。另：09-19/09-20 两个快照是**残缺回填**（行项表 25 行 / 48 行，对 09-22 起的 2,361 行），**09-21 两张表整天缺** | 0 不等于守卫空转 |
| E-19 | ★★ **两张采购表的采集日集合不一样**（09-23 实测）：行项表 09-19 / 20 / **22** / 23，单据表 09-19 / 20 / 23 —— **09-22 那天行项是当天的、单据停在 09-20（两天前）**。在途 = 行项给数量 × 单据给 `status`，而陈旧守卫只量了行项表。单据表冻住 ⇒ 已完成/已作废的单继续被读成「待到货」，而 E-12 说 status=9 的行项 `quantity_receive` 全为 0，算术过滤救不了 —— 最多虚报 104,479 件（真实开口量的 3.7 倍）。今天 `purchase_staleness_days = 3` 连那次 2 天的漂移都不会红——★ **扩表之后也照样不会红**（09-22 单据表 age=2 < 3）：扩表修的是「另一张表根本没人看」，不是「阈值该多大」，两件事别混（见 OQ-7） | 守卫要量两张表里更老的那个 |
...
- ★ **必须让人知道「今天几乎一件都不落在计划窗口内」**：E-13 实测 28,561 件开口在途里 2026-10 只有 345 件、
  之后为 0，整批来自 9 张采购单（E-13a：初稿写「2 张」是错的）。10 月起的计划看到的在途接近空 ——
  这是**数据的形状**，不是「没取到」。
- ★ **陈旧守卫必须量两张表**（E-19）：`purchase_as_of` 取行项表的 `max(_captured_date)`，但 `status` 来自单据表，
  两张表的采集日集合实测不一样。年龄取**更老的那个**判阈值，异常与日志点名是哪张表老了 ——
  行项表停了是「没有新采购」，单据表停了是「状态冻住」，处置不同。返回值仍是行项表的快照日，口径不变。
...
- `as_of()` 的解析结果另缓存 `cache_ttl_s`（默认 300 秒）。
- 效果：21 次 → **3 条 SQL**（as_of 候选 / 在仓批量 / 采购批量），实测 23 ms + 16 ms 量级。
- ★ **销量不批量、不缓存，每个 msku 一条 SQL**（终审 M-2 更正：初稿写「按 `(sid, 窗口)` 批量并缓存同键」，
  代码从来不是这样）。今天成立是因为上面那 21 次里**根本不含销量** —— grid 不取销量，只有 `claim()` 取，
  而 `claim()` 每请求只调一次。**哪天 grid 开始要销量历史，这一条就要重做**，否则 N 个 msku = N 条 SQL。
...
| OQ-7 | ★ **`purchase_staleness_days` 是一个数，管着两张采集节奏不同的表**（Task 7 复核 F5 提出）。单据表是小表（125 单 vs 行项 2,361 行）、采集更稀（09-22 整天没采而行项采了）。阈值该不该按表分开定、单据表的容忍度该更紧还是更松，**未裁定** | 阶段 A 先用同一个保守值（3 天），**留阶段 B 用真实采集节奏校准**——两张表各拍一个数而没有实测节奏支撑，只是把猜测从一处变成两处。★ 阶段 B 校准前先把两张表各自的采集日序列拉满 30 天，别拿 4 天的样本定阈值 |
