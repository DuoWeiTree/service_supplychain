...
改这一行之前跑一次 `uv run pytest -q tests/test_order_store_map_live.py`，四条
...
2. **每个能被认领的 sid 都解析得出 store**——反方向：宇宙是 `msku_bridge`
   ...
   合计 179 个 msku），它们在订单报表里**一单都没有**。认领它们**不是 503**：
   格子照常建起来、`system_units` 留空、标成「不适用」并带上成因与出处。
   认领一个**没登记**的 sid 才是 503，提示是「补一行映射」。
3. **订单报表里仍然没有任何 sid 列**——有了的话整张声明表都要重做。
4. **豁免还没过期**——白名单也要能变红，两个触发器：豁免 sid 名下的 msku 数越过
   `max(登记值 × 1.20, 登记值 + 5)`；或订单报表冒出一个没见过的 `store` 值。
   红了要重新审这条豁免，**不许直接把阈值调大**。
