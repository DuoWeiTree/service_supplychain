...
def test_l7_no_writes_to_clickhouse():
    ...
    # ★ 终审 M-10 扩面到 api/ 与 erp/：`api/__init__.py` 现在已经 import 了
    #   `jobs.refresh_dims` 与 `jobs.scheduler`，离 CH 比以前近；而全仓没有任何
    #   规则禁止 `api/` 直接 import clickhouse_connect（NO_CONNECTIONS 只管
    #   forecast/rules/dim，L6 只管 psycopg2）。已 parked 的盲点是「非字面量表名」，
    #   这是另一条轴：**文件范围**。
    ch_scope = sources("jobs", "api", "erp") + [ROOT / "shared" / "ch_client.py"]
    for p in ch_scope:
